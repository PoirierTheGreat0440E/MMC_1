...
function Topic({id,topic_name,topic_changer}){
    return(<article className="topic" onClick={()=>{topic_changer(id);}}>
        <p>{topic_name}</p>
    </article>);
}
...
function TopicList({topic_list,selected_topic ,topic_identifier}){
    ...
    return(<div className="topic_list">
        ...
        {topic_list.map( (topic) => <Topic key={topic.topic_id} id={topic.topic_id} topic_name={topic.topic_titre} topic_changer={topic_identifier} /> )}
    </div>);
